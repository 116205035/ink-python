"""Focus runtime data structures for :mod:`ink.hooks.focus` (Phase 2 PR6).

Mirrors ink's ``use-focus`` / ``use-focus-manager`` pair (see
``D:/Projects/github/ink/ink-master/src/hooks/use-focus-manager.ts``).

Three pieces live here so :mod:`ink.hooks.focus` can stay focused on the
hook integration glue:

* :class:`FocusHandle` — the per-component handle returned by
  :func:`ink.hooks.focus.use_focus`. Carries a unique id and a
  ``is_focused`` :class:`~ink.core.signal.Signal` consumers can subscribe
...
* :class:`NullFocusManager` — a no-op fallback returned by
  :func:`~ink.core.context.use_context` when a ``use_focus`` consumer is
...
State is held in :class:`~ink.core.signal.Signal` objects so the render
...
def _next_handle_id() -> str:
    ...
    return f"ink-focus-{next(_handle_id_counter)}"


class FocusHandle:
    """A single focusable component's handle.

    Created by :func:`ink.hooks.focus.use_focus` and registered with the
    ...
    is_focused:
        :class:`~ink.core.signal.Signal` of ``bool``. ``True`` while
...
class FocusManager:
    ...
    :func:`ink.hooks.focus.use_focus_manager`. The instance is then
    injected into the subtree via a :class:`~ink.core.context.Provider`
    and read back by :func:`~ink.hooks.focus.use_focus` via
    :func:`~ink.hooks.context.use_context`.
    ...
    * ``enabled=False`` clears every handle's ``is_focused`` signal.
    ...
    def _set_active_index(self, idx: int | None) -> None:
        ...
        snapshot. We don't use :func:`ink.core.signal.batch` here to
...
class FocusManagerHandle:
    """Control surface returned by :func:`ink.hooks.focus.use_focus_manager`.

    Carries the underlying :class:`FocusManager` (for direct access when
    needed) and a ``wrap`` callable that builds a
    :class:`~ink.core.context.Provider` injecting the manager into the
    ...
    * The manager needs to live in a :class:`~ink.core.context.Provider`
    ...
    * Forwarding every manager method on this object keeps the call sites
    ...
    def wrap(self, *children: object) -> Element:
        ...
        ``ink.core.context`` at module load (which would create a cycle
        when ``ink.core.context`` itself ever needs to introspect focus
        state — it currently doesn't, but the laziness is cheap insurance).
        """
        from ink.core.context import Provider  # local import: avoid cycle
        from ink.core.element import Element as _Element
