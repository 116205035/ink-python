...
:func:`ink.render.render`. Walks the active context stack (set up by
:mod:`ink.core.context` and pushed/popped by Provider host elements
in :mod:`ink.core.reconciler`) from top to bottom and returns the
...
def use_context(ctx: Context[T]) -> T:
    ...
    RuntimeError
        ...
        :func:`ink.render.render`. The hook itself doesn't need the
