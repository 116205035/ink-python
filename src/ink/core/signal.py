...
_current_observer: ContextVar[_Observer | None] = ContextVar(
    "ink_current_observer", default=None
)

#: Non-zero when inside one or more ``batch(...)`` calls.
_batch_depth: ContextVar[int] = ContextVar("ink_batch_depth", default=0)
...
_notifying: ContextVar[bool] = ContextVar("ink_notifying", default=False)
...
_current_component: ContextVar[object | None] = ContextVar(
    "ink_current_component", default=None
)
...
def _set_current_component(instance: object | None) -> Token[object | None]:
    ...
    hooks (see :class:`ink.core.component.ComponentInstance`) which
