...
* :func:`use_context` (in :mod:`ink.hooks.context`) walks the stack from
...
class Context(Generic[T]):
    ...
    The default value is returned by :func:`ink.hooks.context.use_context`
...
def get_context_stack() -> list[tuple[int, Any]]:
    ...
    Exposed so :func:`ink.hooks.context.use_context` can scan it
...
def Provider(ctx: Context[T], value: T, *children: Any) -> Element:
    ...
    * pop on unmount
    ...
    see ``value`` through :func:`ink.hooks.context.use_context`.
