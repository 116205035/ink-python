...
(see :data:`pyink.render.ansi.NAMED_COLORS`) spells that ``"gray"`` /
``"grey"`` / ``"blackBright"``. We use ``"gray"`` for file-marker dim
treatment, matching :mod:`pyink.externals.highlighted_code`.
...
def _resolve_source(source: str | Signal[str] | Callable[[], str]) -> str:
    ...
    helpers in :mod:`pyink.externals.streaming_text` and
    :mod:`pyink.externals.markdown`.
    """
...
def _DiffImpl(**props: Any) -> Element:
    ...
    :func:`pyink.externals.markdown._MarkdownImpl` uses).
...
def StructuredDiff(
    ...
    **box_props: Any,
    ...
    before:
        ...
        * ``str`` — static.
        * :class:`pyink.Signal` ``[str]`` — reactive.
