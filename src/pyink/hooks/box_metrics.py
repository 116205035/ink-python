...
* :func:`use_box_metrics` — reactive subscription. Returns a
  :class:`pyink.core.signal.Computed` of :class:`BoxMetrics`; reading
  ``.value`` inside a callable ``Text`` leaf / ``effect`` /
  :class:`pyink.core.signal.Computed` body subscribes to layout updates.

Reactivity plumbing: the underlying ``ref.value`` is a plain
:class:`pyink.core.signal.Ref` (PRD Decision 2 — non-reactive holder). The
hook therefore combines ``ref.value`` with the module-level
:data:`pyink.hooks._box_metrics_runtime.layout_epoch` signal inside a
...
from pyink.core.signal import Computed, computed
from pyink.hooks._box_metrics_runtime import (
    UNMEASURED,
    BoxMetrics,
    layout_epoch,
)
...
if TYPE_CHECKING:
    from pyink.core.signal import Ref
...
def measure_element(ref: Ref[LayoutNode | None]) -> BoxMetrics:
    ...
    ----------
    ref:
        The :class:`pyink.core.signal.Ref` passed as the ``ref`` prop of a
        :func:`pyink.components.Box`. ``ref.value`` is ``None`` before the
...
def use_box_metrics(ref: Ref[LayoutNode | None]) -> Computed[BoxMetrics]:
    ...
    Returns a :class:`pyink.core.signal.Computed` of
    ...
    via :func:`pyink.render.render`. It is also usable outside ``render``
