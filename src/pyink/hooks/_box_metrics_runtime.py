...
Three pieces live here so :mod:`pyink.hooks.box_metrics` can stay focused on
the user-facing API:

* :class:`BoxMetrics` — the immutable measurement result a caller reads from
  :func:`pyink.hooks.box_metrics.measure_element` or the
  :class:`~pyink.core.signal.Computed` returned by
  :func:`pyink.hooks.box_metrics.use_box_metrics`. Carries the four
...
* ``layout_epoch`` — a process-wide :class:`~pyink.core.signal.Signal` of
  ``int`` incremented at the end of every layout pass. ``use_box_metrics``
  reads this signal inside its :class:`~pyink.core.signal.Computed` body so
  the computed re-evaluates after every layout — without it the computed
  would only ever read ``ref.value`` (a plain :class:`~pyink.core.signal.Ref`
...
The epoch is bumped from :mod:`pyink.render.instance` (the render loop's
layout passes). Test code that drives layout directly via
:func:`pyink.layout.layout` can also call :func:`bump_layout_epoch` to keep
...
class BoxMetrics:
    ...
    top-left corner** (matching :class:`pyink.layout.flex.LayoutNode.x` /
